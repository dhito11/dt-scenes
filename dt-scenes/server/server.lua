...
ESX.RegisterServerCallback('dt-scenes:server:GetScenes', function(_, cb)
    cb(scenes)
end)
...
function UpdateAllScenes()
    ...
    MySQL.Async.fetchAll('SELECT * FROM scenes', {}, function(result)
        ...
        TriggerClientEvent('dt-scenes:client:UpdateAllScenes', -1, scenes)
    end)
end
...
RegisterNetEvent('dt-scenes:server:DeleteScene', function(id)
    MySQL.Async.execute('DELETE FROM scenes WHERE id = ?', {id}, function()
        UpdateAllScenes()
    end)
end)

RegisterNetEvent('dt-scenes:server:CreateScene', function(sceneData)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    TriggerEvent("qb-log:server:CreateLog", "scenes", "New Scene Created", "red", "**".. GetPlayerName(source) .. "** (@identifier: *"..xPlayer.identifier.."* | id: *"..source.."*) created a new scene; scene: **"..sceneData.text.."**, where: **" .. sceneData.coords .. "**")
