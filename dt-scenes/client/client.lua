...
RegisterNetEvent('dt-scenes:client:UpdateAllScenes', function(list)
    scenes = list
end)
...
function GetScenes()
    ESX.TriggerServerCallback('dt-scenes:server:GetScenes', function(list)
        scenes = list
    end)
end
        ...
        CreateThread(function()
            while creationLaser do
                ...
                if IsControlJustReleased(0, 38) then
                    creationLaser = false
                    if hit then
                        TriggerServerEvent('dt-scenes:server:CreateScene', data)
                    else
                       TriggerEvent(Lang:t("notify.laser_error"), "error")
                    end
...
function DeleteScene(coords)
    ...
    if closestScene then
        TriggerEvent(("notify.scene_delete"), "success")
        TriggerServerEvent('dt-scenes:server:DeleteScene', closestScene)
    else
       TriggerEvent(Lang:t("notify.scene_error"), "error")
    end
end
